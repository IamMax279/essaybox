...
export default function Nowy({ refetchEssays }: { refetchEssays: () => void }) {
    ...
    const { mutate: checkTransaction } = useMutation({
        mutationFn: async () => {
            const response = await axios.post(
                `${process.env.NEXT_PUBLIC_SERVER_URL}/transaction/get-transaction`,
                { transaction: success },
                { withCredentials: true }
            )
